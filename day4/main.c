...
int main(int argc, char *argv[]) {
    ...
    number[0] = number [1] = 0;
    int i = 0;
    while (i < 100) {                                                       // reads the first line and get all integers
        int c = fgetc(fp);
        if (c == 44 || c == 10) {
    ...
    int c = fgetc(fp);
    while (c != EOF) {
        if (isdigit(c)) {                                           // new number means new matrix :)
            for (int i = 0; i < 5; i++) {
                for (int j = 0; j < 5; j++) {
                    int first = c - 48;
                    c = fgetc(fp);
                    ...
                    while (!isdigit(c) && c != EOF) c = fgetc(fp);
                    table[i][j] = 0;
                }
            }
            check();
...
int solved(int row, int col) {
    ...
    int nbrcol = 0;
    for (int i = 0; i < 5; i++) {
        if(table[row][i] == 1) nbrrow++;
        if(table[i][col] == 1) nbrcol++;
    }
